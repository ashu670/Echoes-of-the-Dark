...
public class ParanormalManager : MonoBehaviour
{
    ...
    IEnumerator ForceEvent(ParanormalEvent e, PlayerSystem player)
    {
        isEventRunning = true;

        yield return StartCoroutine(TriggerWrapper(e, player));

        isEventRunning = false;
    }

    IEnumerator TriggerWrapper(ParanormalEvent e, PlayerSystem player)
    {
        TriggerEvent(e, player);
    ...
    public void ForceTrigger(EventType type, PlayerSystem player)
    {
        ...
        foreach (var e in events)
        {
            if (e.type == type)
            {
                StartCoroutine(ForceEvent(e, player));
                return;
            }
        }
    }
